...
| `src/app/` | The dev shell: `root.tscn` routes to the validation bootstrap under `--test-mode`, else to `battle_viewer.tscn`, and registers the InputMap actions; the viewer plays any battle file (`-- --battle`) with its declared AIs, with the first faction on the mouse by default (H cycles sides then watch mode; `-- --human <faction>` or `-- --watch` at launch) with click mapping generic over action params, and prints the `[Kit] Battle ready:` marker. |
...
| `src/tools/` | Repo-owned checks: `check_scripts.ps1` (compile gate, engine half in `compile_check.gd`); `export_web.ps1` (Web export that proves its files, fetching a standard editor if godot is mono) and `serve_web.ps1` (local server with the right wasm mime). |
