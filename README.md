...
| `examples/battlefield` | Heroes of Might and Magic style, on a painted half-offset-square map (Liberty or Death brick columns, hex adjacency, the odd-q `hex_columns` layout): creature stacks whose count follows their hp pool, per-unit initiative with Wait, one retaliation a round, two-cell flyers that cross obstacles but cannot land on them, ranged shots with a distance penalty, a lich death cloud that hits friends too, Defend, and its own presentation with damage-and-kills hover readouts |
